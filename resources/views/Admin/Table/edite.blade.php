<x-admin-layout>
    <x-slot name="content">
      <form action="{{route('admin.tables.update' , ["table"=>$table->id])}}"  method="post" class="w-full container md:w-2/3 mx-auto h-fit"> 
        @csrf
        @method("PUT")
        <h1  class=" text-white mb-4 text-xl my-2 text-center font-bold">update Table</h1>
        <div class="mb-5">
            <label for="name" class="block mb-4 font-semibold text-sm  text-gray-900 dark:text-white"  >Name</label>
            <input type="text" value="{{$table->name}}" id="name" name="name" class="bg-gray-50 border outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @error('name') 
      ...
      <div class="mb-5"> 
        ...
        <select id="status" name="status"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          ...
          @foreach (\App\Enums\tableStatus::cases() as $item)              
               <option value="{{$item->value}}" @selected($item->value === $table->status) >{{$item->name}} </option>
          @endforeach
      ...
      <div class="mb-5"> 
        ...
        <select id="location" name="location"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          ...
          @foreach  (\App\Enums\tableLocation::cases() as $item)
            <option value="{{$item->value}}" @selected($item->value === $table->location) >{{$item->name}} </option>
          @endforeach
        </select>
        @error('location')
        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{$message}}</p> 
