<x-admin-layout>
    <x-slot name="content">
      <form action="{{route('admin.menus.store')}}" enctype="multipart/form-data" method="post" class="w-full container md:w-2/3 mx-auto h-fit"> 
        @csrf
        <h1  class=" text-white mb-4 text-xl my-2 text-center font-bold">Create Menu</h1>
        <div class="mb-5">
        ...
        <div class="mb-5">
            ...
            <input type="text" value="{{old('description')}}" id="description"  name="description" class="bg-gray-50 h-14 border outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @error('description')
              <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{$message}}</p> 
            @enderror
        </div>
      ...
      <div class="mb-5 ">
        ...
        <input type="number" id="number-input" name="price" value="{{old('price')}}" class="bg-gray-50 border outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="90210"  />
        @error('price')
        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{$message}}</p> 
        @enderror
      </div>
        ...
        <button type="submit" class="p-2.5 text-sm font-medium w-full bg-indigo-500 hover:bg-indigo-700 text-lime-50 rounded-lg" >Create Menu</button>
     </form>
